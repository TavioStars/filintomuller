...
const UsersSheet = ({ open, onOpenChange }: UsersSheetProps) => {
  ...
  const fetchProfiles = async () => {
    ...
    setProfiles((data || []).map((p: any) => ({ ...p, avatar_url: p.avatar_url || null })));
  ...
  const fetchAccessRequests = async () => {
    ...
    if (error) { console.error("Error fetching access requests:", error); return; }
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      // Use admin RPC to update name and role
      const { error: profileError } = await supabase.rpc("admin_update_profile" as any, {
      ...
      if (isAdmin && !userRoles[selectedUser.id]?.some(r => r.role === "admin")) {
        const { error } = await supabase.rpc("add_admin_role", { target_user_id: selectedUser.id });
        if (error) throw error;
      } else if (!isAdmin && userRoles[selectedUser.id]?.some(r => r.role === "admin")) {
        const { error } = await supabase.rpc("remove_admin_role", { target_user_id: selectedUser.id });
        if (error) throw error;
      }

      toast({ title: "Alterações salvas", description: "As informações do usuário foram atualizadas." });
  ...
  const handleApproveAccess = async (request: AccessRequest) => {
    ...
    try {
      ...
      const { error: statusError } = await supabase.rpc("update_account_status", { target_user_id: request.user_id, new_status: "approved" });
      if (statusError) throw statusError;
      toast({ title: "Acesso aprovado", description: `O acesso de ${request.name} foi aprovado.` });
      fetchAccessRequests();
      fetchProfiles();
    } catch (error: any) {
  ...
  const handleDenyAccess = async (request: AccessRequest) => {
    setLoading(true);
    try {
      const { error: requestError } = await supabase.from("access_requests").update({ status: "denied", reviewed_at: new Date().toISOString() }).eq("id", request.id);
      if (requestError) throw requestError;
      const { error: statusError } = await supabase.rpc("update_account_status", { target_user_id: request.user_id, new_status: "denied" });
      if (statusError) throw statusError;
      toast({ title: "Acesso negado", description: `O acesso de ${request.name} foi negado.` });
      fetchAccessRequests();
      fetchProfiles();
    } catch (error: any) {
      toast({ variant: "destructive", title: "Erro ao negar", description: error.message });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleAccess = async (profile: Profile) => {
    ...
    try {
      ...
      const { error } = await supabase.rpc("update_account_status", { target_user_id: profile.id, new_status: newStatus });
      if (error) throw error;
      toast({ title: newStatus === "approved" ? "Acesso retomado" : "Acesso negado", description: `O acesso de ${profile.name} foi ${newStatus === "approved" ? "retomado" : "negado"}.` });
      fetchProfiles();
    } catch (error: any) {
  ...
  const filteredProfiles = profiles.filter((profile) => {
    const matchesSearch = profile.name.toLowerCase().includes(searchTerm.toLowerCase()) || profile.role.toLowerCase().includes(searchTerm.toLowerCase());
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {activeProfiles.map((profile) => {
                      ...
                      return (
                        <Card key={profile.id} className="cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => handleUserClick(profile)}>
                          <CardContent className="p-4">
                            <div className="flex items-center gap-3">
                              ...
                              {isUserAdmin && <Badge variant="default">Admin</Badge>}
                            </div>
                          </CardContent>
                        </Card>
                      );
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {deniedProfiles.map((profile) => (
                        <Card key={profile.id} className="opacity-50 cursor-pointer hover:opacity-70 transition-opacity" onClick={() => handleUserClick(profile)}>
                          <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                              ...
                              <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); handleToggleAccess(profile); }} disabled={loading}>
                                Retomar Acesso
                              </Button>
                            </div>
                          </CardContent>
                      ...
                      <CardContent className="p-4">
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">Solicitou: {request.requested_role}</p>
                            <p className="text-xs text-muted-foreground mt-1">{new Date(request.created_at).toLocaleDateString("pt-BR")}</p>
                          </div>
                          <div className="flex gap-2">
                            <Button onClick={() => handleApproveAccess(request)} disabled={loading} className="flex-1 gap-2" variant="default">
                              <CheckCircle className="h-4 w-4" /> Permitir
                            </Button>
                            <Button onClick={() => handleDenyAccess(request)} disabled={loading} className="flex-1 gap-2" variant="destructive">
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="flex justify-center">
              <Avatar className="h-20 w-20">
                {selectedUser?.avatar_url ? <AvatarImage src={selectedUser.avatar_url} alt={selectedUser.name} /> : null}
            ...
            {selectedUser?.status === "approved" && !userRoles[selectedUser?.id]?.some(r => r.role === "admin") && (
              <div className="pt-4 border-t">
                <Button variant="destructive" className="w-full" onClick={(e) => { e.preventDefault(); if (selectedUser) { handleToggleAccess(selectedUser); setEditDialogOpen(false); } }} disabled={loading}>
                  Negar Acesso
                </Button>
              </div>
            )}
            {selectedUser?.status === "denied" && (
              <div className="pt-4 border-t">
                <Button variant="outline" className="w-full" onClick={(e) => { e.preventDefault(); if (selectedUser) { handleToggleAccess(selectedUser); setEditDialogOpen(false); } }} disabled={loading}>
                  Retomar Acesso
                </Button>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveChanges} disabled={loading}>Salvar alterações</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
